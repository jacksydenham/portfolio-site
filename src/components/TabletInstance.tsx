...
export default function TabletInstance({
  ...
  useFrame(({ clock }, dt) => {
    ...
    isHero && (window as any).hoveredTabletTriggers !== null &&
    categories.some((c) => (window as any).hoveredTabletTriggers!.includes(c));
    
    const hoverActive = (isHovered || heroTriggerMatch) && ((isProjects && !inDelayGate) || isHero);
    ...
    const animatedY = THREE.MathUtils.damp( ref.current.position.y, targetY + hoverLift, 6, dt );
    
    // idle bob
    const bobY = hoverActive
    ? Math.sin(elapsed * bobSpeed + bobPhase) * 0.005
    : 0;
    ref.current.position.set( basePosition.current.x, THREE.MathUtils.lerp(targetY + 1, animatedY, eased) + bobY, basePosition.current.z );
    
    if (hoverActive) {
      if (isHero && setHoveredCategory) { ref.current.rotation.set(0, 0, 0) } 
      else if (isProjects) {
        ref.current.rotation.x = THREE.MathUtils.damp( ref.current.rotation.x, -Math.PI / 2, 6, dt );
        ref.current.rotation.y = THREE.MathUtils.damp( ref.current.rotation.y, -Math.PI, 4, dt );
        ref.current.rotation.z = (ref.current.rotation.z + dt * angularVelocity.current) % (Math.PI * 2);
        angularVelocity.current = THREE.MathUtils.damp( angularVelocity.current, 0.5, 2, dt ); // sonic spin
      }
    } else {
      if (setHoveredCategory && isHero) { ref.current.quaternion.slerp(new THREE.Quaternion(), 0.1) } 
      else {
        ref.current.rotation.x = THREE.MathUtils.damp( ref.current.rotation.x, 0, 6, dt );
        ref.current.rotation.y = THREE.MathUtils.damp( ref.current.rotation.y, -Math.PI, 6, dt );
        ref.current.rotation.z = THREE.MathUtils.damp( ref.current.rotation.z, 0, 6, dt );
      }
    }

    // hide other tablets based on first category
    const shouldHide = isHero &&
    (window as any).hoveredTabletName !== null &&
    ...
    materialRef.current.opacity = THREE.MathUtils.damp( materialRef.current.opacity, targetOpacity, 8, dt );
    materialRef.current.opacity = THREE.MathUtils.damp( materialRef.current.opacity, targetOpacity, 8, dt );
  });
